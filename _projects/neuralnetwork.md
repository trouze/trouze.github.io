...
To follow along or see the data, you can download from [my repository](https://github.com/trouze/UMNStuffs/blob/master/HW4NN.zip) on Github, which also includes the R script to load the data and build the Neural Network. What will follow will be two-fold: a TL;DR (well, sort of) version of what a Neural Network is and how it works, and then an implementation in R for those who may want to learn how to do it in this language. If you truly want to learn about neural nets, I suggest you read the aforementioned NNDL textbook.

### How do Neural Networks, work?

At a high level, Neural Networks are just that, a model of how your neurons work in your brain. The difference here is that it's an emulation of your brain in a computer (not as scary as it sounds). The idea is that, if you were shown the number 5 right now, your eyes would register the number, pass that information to your brain in which certain neurons would fire based on the image and your brain would determine that it is, in fact, a 5 you are seeing.
...
Each of the 784 input values is sent to each of the nodes in the middle (hidden) layer. In our case this represents a 784 by 30 matrix as you'll see we have 30 nodes in the hidden layer, and 10 in the output layer, each of the 10 representing a final determination of the handwritten digit being 0 through 9.
...
Now that we've briefly walked through what steps a network takes to classify a handwritten digit, we must walk through how we train the network to get good at classifying digits correctly. Let's try to do this while also showcasing some of the code to be implemented in R, in order to keep this post on the shorter side.

To train a network, we must give it some data so it can learn. We do this by splitting the dataset. If you've studied statistics in any capacity, you'll likely be familiar with this practice. In our handwritten image dataset we have 70,000 images, so we'll feed our network 60,000 images to learn from and 10,000 to test on. The primary difference between the "learning" digits and the "testing" digits, is that in the learning phase we are able to adjust the weights and biases such that it gets more digits right, this is done through a reduction in what is called a cost function.

#### Stochastic Gradient Descent

To start, we begin with the Stochastic Gradient Descent function. The cost function I referenced above? Gradient descent is a fancy way of saying we minimize the cost function (i.e. minimize how many digit classifications we get wrong). The stochastic portion references the fact that we are estimating gradient descent.

Why are we estimating the gradient? Because to train our neural network we will utilize a process called mini batching. We use mini batching for a number of reasons, but the main ones being that we can train our network using a much smaller amount of computing power (this is by far not a complete [answer](https://stats.stackexchange.com/questions/49528/batch-gradient-descent-versus-stochastic-gradient-descent/68326#68326)).

To begin building this learning part of our network, we must split the training data out into mini-batches of size 10 (meaning each mini batch has 10 handwritten images in it). In R, this is how we'd do it.
...
Within this function, we instantiate an empty list for the weights and biases, and we iterate through each observation in the mini batch, feeding the gradient values (how dark a pixel is) as `x` and what the actual handwritten digit is as `y`.
...
To help understand where we are, we've just taken one observation, we've run in through our network and calculated the weight on each edge, and the activation at each node. Now is where we backpropagate, meaning we determine the weights in which we will classify the digit correctly. We calculate this through the gradient of our cost function, meaning we attempt to minimize the chance our network missclassifies the digit.
...
#### In Closing
If you've made it this far, congratulations, I hope you learned a little bit about how neural networks are implemented and how they learn. If you'd like to try it our for yourself, see the [source code](https://github.com/trouze/UMNStuffs/blob/master/HW4NN.zip). There are a number of helper functions that I didn't go over for the sake of brevity, so they can be found in the R file.
