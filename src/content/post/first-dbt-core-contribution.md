...
description: "My first dbt-core open-source contribution adds unmodified and old state selection methods, making it easy to exclude unchanged views from your DAG."
tags: [dbt, open-source, data-engineering]
---
...
```bash
dbt run -s config.materialized:view,state:unmodified --state ./path/to/artifacts
```
